 ...
 * @author Zidai
 */
@WebServlet(name = "HoraCita", urlPatterns = {"/HoraCita"})
public class HoraCita extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Date fechaCita = new Date(NuevaCita.anio - 1900, NuevaCita.mes - 1,Integer.parseInt(request.getParameter("DiaCita")));
        ...
        for(int i=0;i<listaTodasHoras.size();i++){
            ...
            calF.setTime(fecha);
            calH.setTime(hora);
            Date fechaYHora=new Date(calF.get(Calendar.YEAR)-1900,calF.get(Calendar.MONTH),calF.get(Calendar.DAY_OF_MONTH),calH.get(Calendar.HOUR_OF_DAY),calH.get(Calendar.MINUTE), calH.get(Calendar.SECOND));
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<form action='CitaAgendada?IDpaciente="+idPaciente+"&DIAcita="+diaCita+"' method='post'>");
